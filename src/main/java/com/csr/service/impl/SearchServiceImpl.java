...
public class SearchServiceImpl implements SearchService {
     ...
     */
    @Override
    public List<Charity> fetchNearbyNgoList(
    ...
    @Override
    public List<Charity> fetchNearbyNgoListForRequiredCategory(
            String category, double lat, double lon) {
        List<Charity> sortedNearbyNgoList = fetchNearbyNgoList(lat, lon);
        List<Charity> matchingRequirementNgoList = new ArrayList<Charity>();

        for (Charity ngo : sortedNearbyNgoList) {
            List<CharityEvent> eventList = ngo.getCharityEvent();
            outerloop: for (CharityEvent event : eventList) {
                List<Requirement> requirements = event.getRequirement();
                for (Requirement requirement : requirements) {
                    if (null != requirement.getCategory() && category.equalsIgnoreCase(requirement.getCategory())) {
                        matchingRequirementNgoList.add(ngo);
                        break outerloop;
                    }
                }
            }
        ...
        return matchingRequirementNgoList;
    }
}
